    ...
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[360px_1fr]">
        <aside className="space-y-6">
          ...
          <Card className="border-white/10 bg-white/[0.04] text-slate-100"><CardHeader><CardTitle className="flex items-center gap-2 text-base"><Upload className="h-4 w-4 text-emerald-300" /> Import CSV</CardTitle></CardHeader><CardContent className="space-y-3"><Label>Provider export</Label><Input className="border-white/10 bg-black/20" value={provider} onChange={(e) => setProvider(e.target.value)} /><label className="flex cursor-pointer items-center justify-center gap-2 rounded-lg border border-dashed border-emerald-300/40 bg-emerald-300/5 px-4 py-5 text-sm text-emerald-100 hover:bg-emerald-300/10"><Upload className="h-4 w-4" /> {uploading ? "Importing…" : "Choose CSV"}<input type="file" accept=".csv,text/csv" className="hidden" disabled={uploading} onChange={(e) => { const file = e.target.files?.[0]; if (file) handleUpload(file); }} /></label><p className="text-xs leading-5 text-slate-500">Required columns: product name, total sales, and 7-day sales. Optional dailySalesJson should be a JSON array of date/units objects.</p></CardContent></Card>
        ...
        <main className="space-y-6"><div className="flex items-center justify-between"><div><p className="text-xs uppercase tracking-[0.18em] text-slate-500">Candidate queue</p><h2 className="mt-1 text-xl font-semibold">{candidates.length} imported products</h2></div><div className="text-right text-xs text-slate-500">{isLoading ? "Loading…" : "Scores are deterministic"}</div></div>
          {!candidates.length ? <Card className="border-white/10 bg-white/[0.04] text-slate-100"><CardContent className="flex min-h-64 flex-col items-center justify-center gap-3 text-center"><FileSpreadsheet className="h-10 w-10 text-slate-600" /><p className="font-medium">No product candidates yet.</p><p className="max-w-md text-sm text-slate-500">Import a FastMoss or Kalodata export to calculate the first transparent radar pass.</p></CardContent></Card> : <div className="grid gap-4 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]"><div className="space-y-3">{candidates.map((candidate) => { const m = candidate.metrics as Record<string, unknown>; return <button key={candidate.id} onClick={() => setSelectedId(candidate.id)} className={`w-full rounded-xl border p-4 text-left transition ${selected?.id === candidate.id ? "border-cyan-300/70 bg-cyan-300/10" : "border-white/10 bg-white/[0.03] hover:bg-white/[0.06]"}`}><div className="flex items-start justify-between gap-3"><div><p className="font-medium">{candidate.productName}</p><p className="mt-1 text-xs text-slate-500">{candidate.category || "Uncategorized"} · {candidate.provider}</p></div><span className="rounded-full bg-white/10 px-2 py-1 text-[10px] uppercase tracking-wide text-slate-300">{statusLabels[candidate.reviewStatus] ?? candidate.reviewStatus}</span></div><div className="mt-4 grid grid-cols-3 gap-2 text-xs"><div><p className="text-slate-500">Acceleration</p><p className="mt-1 font-medium text-cyan-200">{String(m.accelerationBand ?? "—")}</p></div><div><p className="text-slate-500">Signals</p><p className="mt-1 font-medium">{String(m.deterministicSignalsMet ?? 0)}/{String(m.deterministicSignalsConsidered ?? 0)}</p></div><div><p className="text-slate-500">Handoff</p><p className="mt-1 font-medium">{candidate.handoffStatus === "not_ready" ? "Blocked" : "Ready"}</p></div></div></button>; })}</div>
            {selected && <CandidateDetail candidate={selected} onUpdate={(data) => updateReview.mutate({ id: selected.id, ...data })} onHandoff={() => handoff.mutate({ id: selected.id })} />}
          </div>}
        </main>
      </div>
    </div>
...
function CandidateDetail({ candidate, onUpdate, onHandoff }: { candidate: any; onUpdate: (data: any) => void; onHandoff: () => void }) {
  ...
  return <Card className="border-white/10 bg-white/[0.04] text-slate-100"><CardHeader><CardTitle className="flex items-start justify-between gap-3"><span>{candidate.productName}</span><span className="text-xs font-normal text-slate-500">ID {candidate.id}</span></CardTitle></CardHeader><CardContent className="space-y-5"><section><h3 className="mb-3 text-xs uppercase tracking-[0.18em] text-slate-500">Raw data and deterministic metrics</h3><div className="grid grid-cols-2 gap-3 md:grid-cols-4">{[["Total sales", raw.totalSales], ["7-day sales", raw.sales7d], ["90-day sales", raw.sales90d], ["Video share", raw.videoSalesPct != null ? `${raw.videoSalesPct}%` : "—"], ["Acceleration", m.accelerationPct != null ? `${metric(m.accelerationPct)}%` : "—"], ["Band", m.accelerationBand], ["Stable days", m.stableDays], ["Latest multiplier", metric(m.latestDayMultiplier)], ["Top-video concentration", m.topVideoConcentrationPct != null ? `${m.topVideoConcentrationPct}%` : "—"], ["Concentration", m.concentrationBand], ["Rating", raw.rating ?? "—"], ["Commission", raw.commissionAfterAdsPct != null ? `${raw.commissionAfterAdsPct}%` : "—"]].map(([label, value]) => <div key={String(label)} className="rounded-lg border border-white/10 bg-black/20 p-3"><p className="text-[11px] text-slate-500">{label}</p><p className="mt-1 text-sm font-medium">{String(value)}</p></div>)}</div><div className="mt-3 rounded-lg border border-white/10 bg-black/20 p-3 text-xs leading-5 text-slate-400"><strong className="text-slate-200">Confidence notes:</strong> {candidate.confidenceNotes || "No confidence notes."}</div></section>
