...
export const radarRouter = router({
  getProfile: protectedProcedure.query(async ({ ctx }) => {
    ...
    const row = await db.select().from(radarProfiles).where(eq(radarProfiles.userId, ctx.user.id)).orderBy(desc(radarProfiles.updatedAt)).limit(1);
  ...
  saveProfile: protectedProcedure.input(z.object({ name: z.string().min(1).max(120), config: profileSchema })).mutation(async ({ ctx, input }) => {
    const db = await getDb();
    if (!db) throw new Error("Database unavailable");
    await db.insert(radarProfiles).values({ userId: ctx.user.id, name: input.name, configJson: JSON.stringify(input.config) });
    return { success: true };
  }),
  listCandidates: protectedProcedure.query(async ({ ctx }) => {
    const db = await getDb();
    if (!db) return [];
    const rows = await db.select().from(radarCandidates).where(eq(radarCandidates.userId, ctx.user.id)).orderBy(desc(radarCandidates.updatedAt));
  ...
  updateReview: protectedProcedure.input(z.object({ id: z.number(), reviewStatus: z.enum(["candidate", "watchlist", "human_review", "avoid", "approved_for_campaign_planning"]), evidenceGateStatus: z.enum(["not_reviewed", "needs_product_intel", "blocked", "approved"]), reviewNotes: z.string().optional(), creatorFit: z.record(z.string(), z.string()).optional() })).mutation(async ({ ctx, input }) => {
    const db = await getDb();
    if (!db) throw new Error("Database unavailable");
    const rows = await db.select().from(radarCandidates).where(and(eq(radarCandidates.id, input.id), eq(radarCandidates.userId, ctx.user.id))).limit(1);
    ...
    await db.update(radarCandidates).set({ reviewStatus: input.reviewStatus, evidenceGateStatus: input.evidenceGateStatus, handoffStatus, reviewNotes: input.reviewNotes ?? null, creatorFitJson: input.creatorFit ? JSON.stringify(input.creatorFit) : rows[0].creatorFitJson }).where(and(eq(radarCandidates.id, input.id), eq(radarCandidates.userId, ctx.user.id)));
    return { success: true, handoffStatus };
  }),
  saveAiBrief: protectedProcedure.input(z.object({ id: z.number(), brief: z.record(z.string(), z.unknown()) })).mutation(async ({ ctx, input }) => {
    const db = await getDb();
    if (!db) throw new Error("Database unavailable");
    await db.update(radarCandidates).set({ aiBriefJson: JSON.stringify({ source: "ai_assisted", ...input.brief }) }).where(and(eq(radarCandidates.id, input.id), eq(radarCandidates.userId, ctx.user.id)));
    return { success: true };
  }),
  handoffToCampaign: protectedProcedure.input(z.object({ id: z.number() })).mutation(async ({ ctx, input }) => {
    const db = await getDb();
    if (!db) throw new Error("Database unavailable");
    const rows = await db.select().from(radarCandidates).where(and(eq(radarCandidates.id, input.id), eq(radarCandidates.userId, ctx.user.id))).limit(1);
    ...
    await db.update(radarCandidates).set({ handoffStatus: "handed_off_to_campaign_planning" }).where(and(eq(radarCandidates.id, input.id), eq(radarCandidates.userId, ctx.user.id)));
